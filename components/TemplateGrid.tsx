    ...
    const communityTemplatesSorted = useMemo(() => {
        ...
        if (communityTagFilter) {
            list = list.filter((t) =>
                t.tags?.some(
                    (tag) =>
                        tag.toLowerCase() === communityTagFilter.toLowerCase()
                )
            );
        }
                    ...
                    <div className="flex flex-wrap items-center gap-2">
                        ...
                        >
                            ...
                            >
                                ...
                                <span>
                                    {communityTagFilter ?? "Filter by tag"}
                                </span>
                                <ChevronDown
                        ...
                        >
                            {(
                                ...
                                const isSelected =
                                    communityFeaturedFilter === value;
                                return (
                                    <button
                                        key={value}
                                        type="button"
                                        onClick={() =>
                                            setCommunityFeaturedFilter(value)
                                        }
                ...
                {communityTemplatesSorted.length > 0 ? (
                    <>
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {paginatedCommunityTemplates.map(
                                (template, index) => (
                                    <div
                                        key={template.id}
                                        style={{
                        ...
                        {communityTotalPages > 1 && (
                            <div className="mt-8 flex items-center justify-center gap-2">
                                <button
                                    onClick={() =>
                                        goToCommunityPage(communityPage - 1)
                                    }
                        ...
                        <div className="mt-4 text-center text-sm text-gray-500">
                            ...
                            {Math.min(
                                communityEndIndex,
                                communityTemplatesSorted.length
                            )}{" "}
                            of {communityTemplatesSorted.length} templates
                        </div>
                    </>
                ) : (
                    <div className="card rounded-2xl p-12 text-center">
